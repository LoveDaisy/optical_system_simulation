...
% OUTPUT
%   p:          2*2*n array, [z1, r1; z2, r2], 1 for entrance pupil; 2 for exit pupil
...
p(1, 2, :) = (front_sys_mat(2, 2, :) - front_sys_mat(1, 2, :) .* ...
    front_sys_mat(2, 1, :) ./ front_sys_mat(1, 1, :)) * obj.surfaces(1).ah;

n0 = back_sys.surfaces(1).glass.getRefractiveIndex(d_line);
back_sys_mat = back_sys.makeGaussianSystemMatrix(d_line, n0);
p(2, 1, :) = -back_sys_mat(2, 1, :) ./ back_sys_mat(1, 1, :) - t1 + t2;
p(2, 2, :) = (back_sys_mat(2, 2, :) - back_sys_mat(1, 2, :) .* ...
    back_sys_mat(2, 1, :) ./ back_sys_mat(1, 1, :)) * obj.surfaces(1).ah;
end